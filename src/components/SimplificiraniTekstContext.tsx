...
export const SimplifiedTextContext = createContext<SimplifiedTextContextType | undefined>(undefined);

export const useSimplifiedTextContext = () => {
    const context = useContext(SimplifiedTextContext);
    if (!context) {
        throw new Error("useSimplifiedTextContext must be used within a SimplifiedTextProvider");
    }
    return context;
};

export const SimplifiedTextProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    return (
        <SimplifiedTextContext.Provider value={{ simplifiedTexts, addSimplifiedText }}>
            {children}
